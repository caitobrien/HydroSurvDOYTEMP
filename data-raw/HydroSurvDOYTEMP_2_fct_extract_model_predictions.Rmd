...
generate_newdata <- function(df,model_name) {
  ...
  newdata <- expand_grid(
  mean.tempz = (seq(from = 6, to = 15, by = 0.1) - scale.x)/scale.sd, #following range set by JG in first shiny edition 
  transport = c(0, 1),
  year = years,
  n = 1
    )
...
all <- list(doy = list(), temp = list(), observed = list()) # Initialize 'all' as an empty data frame
...
for (data_file in data_files) {
  ...
  for (model_file in models) {
    ...
    df.pred <- add_predictions(newdata_info,
                               model_info
                              )
    ...
    if (grepl("doy", model_info$model_name)) {
      all$doy <- append(all$doy, list(df.pred))
    } else if (grepl("temp", model_info$model_name)) {
      all$temp <- append(all$temp, list(df.pred))
    }
  }
}
...
df_aggregated_observed<-df_observed %>% 
  select(year,doy,transport, n, mean.temp, sar.pit, sar.pit.lo, sar.pit.hi, rear_type,  species)
