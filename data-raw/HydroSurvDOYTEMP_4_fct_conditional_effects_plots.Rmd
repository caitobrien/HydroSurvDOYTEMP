  ...
  for (model_file in model_files) {
    ...
    if (is_doy) {
      ...
      p_DOY <- plot(c_eff, plot = FALSE)[[1]] +
        labs(x = "Day-of-year (DOY)", y = "Probability of survival", title = "Day of year passage at LGR") +
      ...
      # Temperature
      # temp rescale
      tempz_xaxis <- (seq(6, 20, 2) - mean(df$mean.temp, na.rm = TRUE)) / sd(df$mean.temp, na.rm = TRUE)
      temp_lab <- as.character(seq(6, 20, 2))

      p_temp <- plot(c_eff, plot = FALSE)[[1]] +
        labs(x = "Temperature (°C)", y = "Probability of survival", title = "River temperature at LGR") +
        geom_line(color = "steelblue4")+
        theme_minimal() +
        theme(
          panel.background = element_rect(color = "transparent"),
