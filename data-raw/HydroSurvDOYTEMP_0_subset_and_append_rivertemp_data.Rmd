...
```{r setup, eval=FALSE}
BON.temp.raw<-read_csv(here::here("data-raw", "rivermg_1704223998_133.csv")) #update to pull from CBR link
...
df<- df%>% 
  ...
  # ) %>% 
  left_join(y= select(BON.temp.data, BON.temp, CY, DOY), by = c("year" = "CY", "doy" = "DOY")) %>% 
