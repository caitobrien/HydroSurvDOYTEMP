...
#save models
saveRDS(mod_doy, file = here::here("data-raw/models/prior", paste0("mod_", gsub("_data_subset.csv", "_doy.rds", filename))))
saveRDS(mod_temp, file = here::here("data-raw/models/prior", paste0("mod_", gsub("_data_subset.csv", "_temp.rds", filename))))
}
...
mod_natural_chinook_doy_prior <- brm(alive | trials(n) ~ doyz + I(doyz^2) + transport + doyz:transport + I(doyz^2):transport + (1 + doyz + I(doyz^2) + transport | year), 
                   data = df.agg, 
                   family = binomial(link = "logit"), 
                   chains = 3,
                   cores = 3,
                   prior = bprior,
                   file = here("data-raw/models", "mod_natural_chinook_doy_prior")
                   )

#check summary
summary(mod_natural_chinook_doy_prior)
...
pp_check(mod_natural_chinook_doy_prior)
prior_summary(mod_natural_chinook_doy_prior)
```
...
r<-loo(mod1, mod_natural_chinook_temp)
kable(r$diffs)
