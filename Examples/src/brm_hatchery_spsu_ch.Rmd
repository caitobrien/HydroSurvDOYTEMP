...
#---or use steps below
DATA <- read.csv(file = here::here("Examples/data/", "dsynth_2021-03-02.spsu.subset.csv"), stringsAsFactors = FALSE, na.strings = c("", "NA"))
...
```{r mod_binomial}
#rm(mod.1)
mod.hatchery_chinook_DOY <- brms::brm(alive | trials(n) ~ doyz + I(doyz^2) + transport + doyz:transport + I(doyz^2):transport + (1 + doyz + I(doyz^2) + transport | year), 
                   data = df.agg, 
                   family = binomial(link = "logit"), 
                   chains = 3,
                   cores = 3,
                   file = here::here("Examples/results", "mod.hatchery_chinook_DOY")
                   )
summary(mod.1.1)
...
# salmon
scale.DOY.sal <- scale(df.ChSSHRT$doy) 
scale.x.sal <- attr(scale.DOY.sal, "scaled:center")
scale.sd.sal <- attr(scale.DOY.sal, "scaled:scale")
...
newdoyz.sal <- ((90:160) - scale.x.sal) / scale.sd.sal 
...
df.linpred<-newdata.full %>% 
  tidybayes::add_linpred_draws(mod.hatchery_chinook_DOY, re_formula = NULL, allow_new_levels = TRUE, ndraws = 10, transform =TRUE) %>% 
  tidybayes::median_qi() %>% 
  rename(SAR = .linpred,
         SAR.lo = .lower,
         SAR.hi = .upper) 
df.linpred <- df.linpred %>% 
  mutate(doy = (df.linpred$doyz * scale.sd.sal) + scale.x.sal,
...
write.csv(all, row.names = F, here("Examples/data", "ChSSRT_mod_predict.csv"))
