...
df.pred<-newdata %>% 
  tidybayes::add_linpred_draws(mod_wild_steelhead_doy, re_formula = NULL, allow_new_levels = TRUE, ndraws = 10, transform =TRUE) %>% 
  tidybayes::median_qi() %>% 
  rename(SAR = .linpred,
         SAR.lo = .lower,
         SAR.hi = .upper) %>% 
...
model_folder<- here::here("data/models")
...
create_model_tables <- function(model_list, output_directory) {
  for (i in seq_along(model_list)) {
    ...
    tbl <- as.data.frame(fixef(model_list[[i]]))
    rownames(tbl) <- c("Intercept", "Day-of-years (DOY)", "DOY^2", "Transport", "DOY:Transport", "doyz2:transport")
    #create table
    table <- tbl %>%
...
```{r summary_table_single}
tbl<-as.data.frame(fixef(mod_wild_chinook_doy)) 
...
# Save the HTML kable table to a file
write_file(table, here::here("inst/app/www", "table.html"))
```
...
tabsetPanel(
  ...
  tabPanel("Natural-origin Chinook",
           includeHTML("inst/app/www/table_mod_wild_chinook_doy.html"),
           br(),
           includeHTML("inst/app/www/table_mod_wild_chinook_temp.html")),
  ...
  tabPanel("Natural-origin Steelhead",
           includeHTML("inst/app/www/table_mod_wild_steelhead_doy.html"),
           br(),
           includeHTML("inst/app/www/table_mod_wild_steelhead_temp.html")),
