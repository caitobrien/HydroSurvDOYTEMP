...
df.pred<-newdata %>% 
  ...
  rename(SAR = .linpred,
         SAR.lo = .lower,
         SAR.hi = .upper) %>% 
  mutate(TI = ifelse(transport == 1, SAR[transport == 1] / SAR[transport == 0], NA),
