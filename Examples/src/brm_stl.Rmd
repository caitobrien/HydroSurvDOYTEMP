...
#---or use steps below
DATA <- read.csv(file = here::here("Examples/data/", "dsynth_2021-03-02.Sthd.subset.csv"), stringsAsFactors = FALSE, na.strings = c("", "NA"))
...
# data.pred<-read.csv(here::here("Examples/data", "ChSSRT_mod_predict.csv"))
# data.pred$transport<-as.factor(data.pred$transport)
# data.pred$year<-as.factor(data.pred$year)
```
...
df.agg <- df.STL %>% 
  ...
  group_by(year, doy, doyz, transport, t_rear_type) %>% 
  mutate(n = n()) %>% 
 ...
 # mutate(year = as.factor(year)) %>% 
  ...
  arrange(year, doy, doyz, transport,t_rear_type) %>% 
  mutate(doy.bin = cut(doy, breaks = seq(90, 160, 10), include.lowest = TRUE)) %>% 
  group_by(year, transport, t_rear_type, doy.bin) %>% 
...
mod.hatchery_steelhead_DOY <- brms::brm(alive | trials(n) ~ doyz + I(doyz^2) + transport + doyz:transport + I(doyz^2):transport + (1 + doyz + I(doyz^2) + transport | year), 
                   data = df.agg.H, 
                   family = binomial(link = "logit"), 
                   chains = 3,
                   cores = 3,
                   file = here::here("Examples/results", "mod.hatchery_steelhead_DOY")
                   )

mod.wild_steelhead_DOY <- brms::brm(alive | trials(n) ~ doyz + I(doyz^2) + transport + doyz:transport + I(doyz^2):transport + (1 + doyz + I(doyz^2) + transport | year), 
                   data = df.agg.W, 
                   family = binomial(link = "logit"), 
                   chains = 3,
                   cores = 3,
                   file = here::here("Examples/results", "mod.wild_steelhead_DOY")
                   )
...
df.linpred<-newdata.full %>% 
  tidybayes::add_linpred_draws(mod.wild_steelhead_DOY, re_formula = NULL, allow_new_levels = TRUE, ndraws = 10, transform =TRUE) %>% 
  tidybayes::median_qi() %>% 
  rename(SAR = .linpred,
         SAR.lo = .lower,
         SAR.hi = .upper) 
df.linpred <- df.linpred %>% 
  mutate(doy = (df.linpred$doyz * scale.sd) + scale.x,
...
all<-rbind(df.pred.post.w,df.pred.post.h)
