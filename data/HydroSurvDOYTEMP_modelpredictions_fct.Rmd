...
import_data <- function(filename) {
  df<-read_csv(here("Examples/data/HydroSurvDOYTEMP_data_subset", filename))
  return(df)
}
...
import_model <- function(filename) {
 model<- readRDS(here::here("Examples/results", filename))
...
append_obsdata_to_predictions <- function(df_pred, df_agg, model_name) {
  ...
  species <- ifelse(grepl("chinook", model_name, ignore.case = TRUE), "Steelhead", "Chinook")
